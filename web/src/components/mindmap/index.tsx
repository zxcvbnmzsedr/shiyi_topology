...
type ID3SVGElement = d3.Selection<SVGElement,
...
export class Markmap {
    ...
    styleNode: d3.Selection<HTMLStyleElement,
        IMarkmapFlexTreeItem,
        HTMLElement,
        IMarkmapFlexTreeItem>;
...
`;
    ...
    renderData(originData?: INode): void {
        ...
        const nodeEnter = node.enter().append('g')
            .on('click', this.options.nodeClick)
...
// Update the links
    ...
    static create(
        ...
        const mm = new Markmap(svg, opts, id);
        if (data) {
            mm.setData(data);
...
function handleNodes(root) {
    ...
    if (root.child && root.child.length > 0) {
        ...
        for (const c of root.child) {
            node.c.push(handleNodes(c))
        }
    }
    return node;
}

const MindMap = ({root, nodeClick}) => {
